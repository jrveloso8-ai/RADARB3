...
export function UnifiedGexBarreirasView({ 
  ...
  const strikesData: StrikeDerivativesData[] = useMemo(() => {
    ...
    // Fator de escala temporal (DTE):
    // DTE curto (3-10d) = Gamma muito pontudo e concentrado no ATM; OI menor
    // DTE médio (17-45d) = Alta liquidez, maior OI acumulado
    // DTE longo (80-108d) = Gamma espalhado, IV mais estático
    const dteFactor = Math.sqrt(30 / Math.max(1, currentExp.dte));
    const oiScale = currentExp.isLiquid ? 1.0 : currentExp.dte <= 10 ? 0.35 : 0.65;
    ...
    for (let i = 0; i < numStrikes; i++) {
      ...
      // Open Interest calibrado por vencimento
      ...
      // Gamma institucional ponderado por DTE (Spot^2 * 100)
      const gamma = (Math.exp(-Math.pow(dist, 2) / (0.003 / dteFactor)) / (spotPrice * 0.15)) * dteFactor;
      const dollarGexCall = (callOi * 100 * (spotPrice * spotPrice) * gamma * 0.00000001); // $M
      const dollarGexPut = (-putOi * 100 * (spotPrice * spotPrice) * gamma * 0.00000001); // $M
      ...
      // Curva de Volatilidade Implícita (IV Skew) calibrada por DTE
  ...
  // Totais & Métricas Institucionais
  ...
  const maxPainX = getX(maxPain);

  return (
    <div className="space-y-6">
      
      {/* Top Back Navigation Bar (se aplicável) */}
      ...
      <div className="bg-[#0c1322] border border-cyan-500/30 p-5 rounded-2xl space-y-4 shadow-xl">
        ...
        {/* Grade de Vencimentos Oficiais Tastytrade (DTEs Calendário Real) */}
        ...
        {/* 6 Cartões Métricos Institucionais */}
        ...
        <div className="bg-[#0c1322] border border-gray-800 p-5 rounded-2xl space-y-4 shadow-xl">
          ...
          <div className="w-full bg-[#040711] p-3 rounded-xl border border-gray-900 overflow-hidden relative">
            <svg viewBox={\`0 0 \${chartW} \${chartH}\`} className="w-full h-auto">
              ...
              {/* Linha Vertical do Zero Gamma Flip */}
              <line
                x1={flipX}
                y1={padT + 12}
                x2={flipX}
                ...
                if (gexSubView === 'calls_vs_puts') {
                  ...
                  return (
                    <g key={s.strike} className="transition-all hover:opacity-80">
                      {/* Barra Call GEX (Verde para cima) */}
                      {s.callGex > 0 && (
                        <rect
                          x={x - barWidth / 2}
                          y={yCall}
                      ...
                      {/* Barra Put GEX (Vermelha para baixo) */}
                      {s.putGex < 0 && (
                        <rect
                          x={x - barWidth / 2}
                          y={yPut}
      ...
      {/* 3. DISTRIBUIÇÃO DE VOLUME & OPEN INTEREST POR STRIKE (BARREIRAS DE OPÇÕES) */}
        ...
        <div className="bg-[#0c1322] border border-gray-800 p-5 rounded-2xl space-y-4 shadow-xl">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-gray-800 pb-3">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                Paredes de contratos institucionais abertos (Puts à esquerda em Rosa, Calls à direita em Ciano).
              </p>
            </div>
            ...
            {strikesData.slice(4, 21).map((s) => {
              ...
              return (
                <div key={s.strike} className={\`grid grid-cols-12 items-center text-xs font-mono py-1 rounded \${isAtm ? 'bg-cyan-950/30' : ''}\`}>
                  {/* Put Bar (Right aligned, growing to the left) */}
                  ...
                  {/* Call Bar (Left aligned, growing to the right) */}
      ...
      {/* 4. IMPLIED VOLATILITY SKEW (SMILE / SMIRK DE VOLATILIDADE) */}
      ...
      {/* 5. PAINEL INSTITUCIONAL: PILARES OPERACIONAIS DO GEX & ENTRADA SNIPER */}
...
fs.writeFileSync(unifiedViewPath, newUnifiedCode.trim(), 'utf8');
console.log('Updated UnifiedGexBarreirasView.tsx with dynamic expirations and back navigation');
